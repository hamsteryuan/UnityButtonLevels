...
//有关于第四层元素的颜色与选中控制
//颜色可以调
//注意，任何函数请前往ThirdButtonListner(第三层的按钮控制)
public class FourLevelButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    private FourLevelButton[] allButtons; // 所有按钮的引用
    ...
    void Awake()
    {
        ...
        // 获取场景中所有 FourLevelButton 组件
        allButtons = FindObjectsOfType<FourLevelButton>();
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        foreach (FourLevelButton button in allButtons)
        {
    ...
    void OnDisable()
    {
        // 如果按钮处于选中状态，取消选中并恢复为正常颜色
        if (isSelected)
        {   
            isSelected = false;
            buttonImage.color = normalColor;  // 恢复为正常颜色
        }
    }
}